...
The real service-topology data source (service mesh / Backstage /
OpsLevel / Cortex) is deferred to Phase 9 or later. Phase 2 ships a
registered stub that emits a typed
:class:`NotOptedInServiceTopologySlice` with ``opted_in=False,
reason="phase_9_or_later"`` and ``confidence="high"`` — absence is the
data (S6-04 ``NotOptedInExternalDocsSlice`` precedent: the probe
successfully *determined* the feature is not opted in; the state lives
in the slice, the confidence reports the quality of the determination).
...
``repo.config.get("service_topology")`` inside ``run`` — never via
subclass. Composition over inheritance; Open/Closed at the file boundary.
