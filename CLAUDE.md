...
- `BLOCKED` / `BLOCKED-PARTIAL` — explicit precondition not met; check the attempt log for the resolution path. As of 2026-07-26 **four** stories are `BLOCKED` — phase-3 `S6-04` (remediation orchestrator), phase-6.5 `S2-05` (canary seed shim), and phase-7 `S3-02` + `S3-03` (npm plugin directory not yet built) — and phase-4 `S7-10` is `BLOCKED-PARTIAL` (17 of 18 ACs done; AC-18's registry-count fence needs the `@register_snapshot_kind` / `@register_delta_rule` registries, which phase-3 `S6-06` did not ship — it landed the minimal inline-dispatch scaffold instead. `grep -rn 'register_snapshot_kind' src/` returns 0 hits). **`S7-10` gates further phase-4 Step-7 work.** The phase-3 engine layer is **unblocked** — the `RecipeEngine.apply` 2-tuple-vs-Protocol contradiction that previously `BLOCKED` `S5-02` and `S5-03` was resolved by phase-3 [ADR-0014](docs/phases/03-vuln-deterministic-recipe/ADRs/0014-recipe-engine-surfaces-transform-via-transform-registry.md): a `RecipeEngine` surfaces its produced `Transform` via a constructor-injected `TransformRegistry` (`apply` stays `-> RecipeOutcome`). Story `S5-01b` shipped the `TransformRegistry` GREEN; `S5-02` shipped the `NpmLockfileRecipeEngine` GREEN (`src/codegenie/transforms/engines/npm_lockfile.py` — the production day-1 deterministic lockfile-edit engine); `S5-03` shipped the `OpenRewriteRecipeEngine` scaffold GREEN (`src/codegenie/transforms/engines/openrewrite.py` — the scaffolded second day-1 engine that pays ADR-0009's "two implementations day 1" rent; widens `JailedEnv` additively with `JvmEnv` and ships a Phase-7-preview Dockerfile-base-image-swap fixture).
