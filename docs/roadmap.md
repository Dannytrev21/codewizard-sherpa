...
| 7.5 | **Multi-language foundations + Python** *(preamble to Phase 8)* | vuln + migration | `LanguagePack` total-value language contract, `tests/conformance/` suite, Python gather layer (Layer A/B probes), `vulnerability-remediation--python--pip` plugin; the language axis proven extension-by-addition | |
