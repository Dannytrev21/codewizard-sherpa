...
- **Languages, like task classes, are introduced one at a time.** Node/TypeScript first (Phases 1–7), Python second (Phase 7.5). The second language is added as a `LanguagePack` value with no edits to the first — proving the *language* axis extends by addition, the mirror of the task-class proof.
...
| 7 | **Add migration task class (Chainguard distroless)** | vuln + migration | Second plugin (`distroless-migration--node--npm`); proves no edits to existing plugins or stable behavior, with bounded ADR-backed primitives allowed when genuinely cross-task | ✅ [07-migration-task-class](phases/07-migration-task-class/) |
| 7.5 | **Multi-language foundations + Python** *(preamble to Phase 8)* | vuln + migration | `LanguagePack` total-value language contract, `tests/conformance/` suite, Python gather layer (Layer A/B probes), `vulnerability-remediation--python--pip` plugin; the language axis proven extension-by-addition | ✅ [07.5-multi-language-foundations-python](phases/07.5-multi-language-foundations-python/) |
| 8 | Hierarchical Planner + pre-rendered hot views | vuln + migration | Planning supervisor, Redis hot views, MCP-style stdio Skills server | ✅ [08-hierarchical-planner-hot-views](phases/08-hierarchical-planner-hot-views/) |
| 9 | Durable workflow envelope — Temporal | vuln + migration | Temporal workflows + activities, Postgres checkpointer, temporal-ui | ✅ [09-temporal-durable-workflow](phases/09-temporal-durable-workflow/) |
...
|---|---|---|
| 0–2 (shipped) | Unit + per-probe golden + adversarial + fence + smoke (`tests/smoke/test_cli_end_to_end.py`) + portfolio sweep (one mode). **Structural-defense retrofit (2026-05-19):** per-submodule cold-start fence (`tests/fence/test_per_submodule_cold_start.py`) catches circular imports the shared pytest interpreter hides; probe-context conformance fence (`tests/fence/test_probe_context_conformance.py`) pins the coordinator-built ctx against the frozen ADR-0007 `ProbeContext` surface; smoke `test_no_probe_errors_in_smoke_run_record` asserts no probe in the run record reports `exit_status="error"` (coordinator failure-isolation otherwise hides AttributeError-class drift). Added after the BudgetingContext `output_dir` drift + plugins.manifest circular-import incidents. | Foundational; metamorphic cache pair pinned in smoke; structural defenses catch composition-shape drift that unit tests (which use the contract type directly) and `import codegenie` cold-start (which loads the empty top-level only) both miss |
...
| **7.5** | (a) **`tests/conformance/` tier** — a parameterized language-conformance suite every registered language is auto-enrolled in; each language ships a mandatory fixture repo + golden, and a capability that passes `mypy` but is semantically broken (e.g. a stub search adapter) fails conformance; (b) **mandatory per-language golden fixtures** under `tests/golden/languages/{language}/`; (c) **extension-by-addition discipline reframe** ([ADR-0043](production/adrs/0043-extension-by-addition-means-no-silent-edits.md)) — Phase 7's byte-edit allowlist is the *last* per-phase enumerated list; new frozen surfaces use a contract + snapshot test (the probe-ABC pattern), not allowlist rows | Phase 7.5 is the first phase to add a second target language; conformance is the only tier that catches "capability slot filled but semantically broken", and the ADR-0043 negative commitment stops per-phase allowlist accretion before Phases 8–16 each add their own |
| **8+** | Per-phase additions follow the same pattern: add rows to `tests/e2e/`, add binaries to `tests/contract/`, add scenarios to `bench/{task-class}/`, add properties for new invariants | Extension by addition applies to the test architecture too |
...
## Phase 7.5 — Multi-language foundations + Python *(preamble to Phase 8 — second target language, extension by addition)*

**Scope.** The second *target language* is introduced — Python — and, as with Phase 7's second task class, the introduction itself is the test: Python lands as **new probes** (Python Layer A/B), **new dep-graph strategies** (pip / poetry / uv), a **new grammar** (`tree-sitter-python`), a **language search adapter** ([ADR-0032](production/adrs/0032-language-search-adapters.md)), and a **new plugin** (`vulnerability-remediation--python--pip`) — with no edits to the Node/TypeScript probes or plugins. The phase first lands the **`LanguagePack` contract**: a total, frozen value with one required field per capability a language must supply (grammar, project detector, Layer A probes, package managers, dep-graph strategies, search adapter). A language cannot be partially registered — an incomplete `LanguagePack(...)` fails `mypy --strict`; a `LanguagePack` whose capabilities are present-but-broken fails the new `tests/conformance/` suite. `register_language()` fans the pack out into the existing decomposed registries (`@register_probe`, `@register_dep_graph_strategy`, the grammar kernel) so shipped Phase 1–7 code is not edited. TypeScript/JavaScript is retrofitted as `LanguagePack` #1; Python is `LanguagePack` #2, which validates the abstraction. The `distroless-migration--python--pip` plugin is deliberately deferred to a fast-follow.

**Discipline reframe.** Phase 7.5 also lands [ADR-0043](production/adrs/0043-extension-by-addition-means-no-silent-edits.md), which restates "extension by addition" as **"no *silent* edits."** Edits the compiler or a fence fully polices — adding a `Literal` member, a struct field, an import line — are the enforcement mechanism, not violations. ADR-0043 makes Phase 7's 10-row byte-edit allowlist the **last** per-phase enumerated list — new frozen surfaces use a contract + snapshot test (the probe-ABC pattern) instead — and introduces a sanctioned **migration** path: a loud, reviewed, all-at-once horizontal sweep with the conformance and golden suites as the safety net, for legitimate cross-cutting change. The freeze discipline (narrow / earned / provisional) and the two judgement-only problems — duplication and freeze-too-early — are addressed as standing review criteria in the design-pipeline skills.

**Tooling & setup.** External: `tree-sitter-python` (PyPI wheel), `scip-python` for the Python search adapter. Python dep-graph extraction reads `pyproject.toml`, `requirements.txt`, `poetry.lock`, `Pipfile.lock`, and `uv.lock`. No new runtime services.

**Testing.** The full Node/TypeScript regression suite (Phases 1–7) runs as a hard gate — proving the second language did not edit or break the first. The new `tests/conformance/` tier (see §"Test architecture evolution") gates every registered language; each language ships a mandatory fixture repo + golden under `tests/golden/languages/{language}/`. The category-based extension-by-addition fence replaces Phase 7's enumerated allowlist and is exercised against a planted silent edit.

**Plugin / contract framing.** Adding Python touches only new files plus the sanctioned additive categories ([ADR-0043](production/adrs/0043-extension-by-addition-means-no-silent-edits.md)): a new `LanguagePack` module, a one-line entry in the language-registry collection point, additive schema `$ref`s, and additive `Literal` / registry members. The `LanguagePack` contract is the one sanctioned shared-file surface — growing it (a genuinely new capability category) breaks every existing pack until updated, which is compiler-enforced and exactly the desired behaviour.

**Exit criteria.** Python and TypeScript both run from the same gather + plugin orchestration. The Node/TypeScript regression suite is unchanged and green. Every registered language passes `tests/conformance/`. An incomplete `LanguagePack` fails `mypy --strict`; the category-based fence rejects a planted silent edit. The `vulnerability-remediation--python--pip` plugin produces a real diff on a vulnerable Python fixture repo.
