...
| **7.5** | (a) **`tests/conformance/` tier** — a parameterized language-conformance suite every registered language is auto-enrolled in; each language ships a mandatory fixture repo + golden, and a capability that passes `mypy` but is semantically broken (e.g. a stub search adapter) fails conformance; (b) **mandatory per-language golden fixtures** under `tests/golden/languages/{language}/`; (c) **category-based extension-by-addition fence** (`tests/fence/test_no_silent_edits.py`) — replaces the per-phase enumerated byte-edit allowlists with a check on edit *category* ([ADR-0043](production/adrs/0043-extension-by-addition-means-no-silent-edits.md)) | Phase 7.5 is the first phase to add a second target language; conformance is the only tier that catches "capability slot filled but semantically broken", and the category fence stops per-phase allowlist accretion before Phases 8–16 each add their own |
...
**Discipline reframe.** Phase 7.5 also lands [ADR-0043](production/adrs/0043-extension-by-addition-means-no-silent-edits.md), which restates "extension by addition" as **"no *silent* edits."** Edits the compiler or a fence fully polices — adding a `Literal` member, a struct field, an import line — are the enforcement mechanism, not violations. ADR-0043 replaces the per-phase enumerated byte-edit allowlists (Phase 7's 10-row list) with a single **category-based fence**, and introduces a sanctioned **migration** path — a loud, reviewed, all-at-once horizontal sweep with the conformance and golden suites as the safety net — for legitimate cross-cutting change.
