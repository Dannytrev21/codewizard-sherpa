...
- `src/codegenie/coordinator/validator.py` — `_ProbeOutputValidator` Pydantic v2 model: `model_config = ConfigDict(frozen=True, extra="forbid")`; `schema_slice: dict[str, JSONValue]` with recursive `JSONValue = Union[None, bool, int, float, str, list["JSONValue"], dict[str, "JSONValue"]]` (`bool` precedes `int` — Pydantic v2 `Union` ordering, otherwise `True` coerces to `1`); `confidence: Literal["high", "medium", "low"]`. Field-validator on `schema_slice` walks every key at every nesting depth (iterative, not recursive — must not raise `RecursionError`) and raises `SecretLikelyFieldNameError` when any key matches the **ADR-0010 §Decision** regex `SECRET_FIELD_PATTERN = re.compile(r"(?i)^.*(secret|token|password|credential|api[_-]?key|auth[_-]?token|bearer|access[_-]?key|private[_-]?key).*$")`. Pydantic v2 wraps the typed error into a `pydantic.ValidationError`; the original is retrievable via `errors()[0]["ctx"]["error"]` or `__cause__`. `SECRET_FIELD_PATTERN` is module-level — single source of truth, re-imported by `output/sanitizer.py` for ADR-0008's defense-in-depth pass. Lazy-imported by the coordinator (`pydantic` must not appear in `sys.modules` from `import codegenie.coordinator`).
...
- [ ] `tests/unit/test_probe_output_validator.py` covers: the `JSONValue` rejection matrix (`bytes`, `bytearray`, `Callable`, `tuple`, `set`, `datetime`, `Path`, `Decimal`, custom object) at both top level and depth ≥ 2; secret-field-name rejection at every regex alternative AND at depth ≥ 3 through `list[dict]`; `Literal["high","medium","low"]` enforcement with parametrized negative-space (casing, trailing-space, extended values, non-string types); `extra="forbid"` rejection; frozen-mutation rejection; `model_validate(asdict(probe_output))` round-trip; `bool`-preserves-type (not coerced to `int`); empty `schema_slice={}` accepted; `SECRET_FIELD_PATTERN` importable as `re.Pattern`; `validator.py` does not import from `codegenie.probes` (ADR-0007 seam); `import codegenie.coordinator` does not pull `pydantic` into `sys.modules`; recursion-safe at depth 200. See S3-02 ACs AC-1..AC-19 for the full matrix.
