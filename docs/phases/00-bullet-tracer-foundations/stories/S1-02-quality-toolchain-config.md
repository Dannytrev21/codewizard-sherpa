...
- [x] AC-1: `pyproject.toml` contains `[tool.ruff]` with `target-version = "py311"`, `line-length = 100`, and `[tool.ruff.lint]` selecting at minimum `E`, `F`, `I` (imports), `B` (bugbear), `UP` (pyupgrade), and `T20` *or* `T201` (no-`print` — either expression is acceptable; `T20` is the family that includes `T201`); and `[tool.ruff.format]` table is declared. The T201 rule must remain *effective for `src/`* — neither `[tool.ruff.lint.ignore]`, `[tool.ruff.lint.extend-ignore]`, nor `[tool.ruff.lint.per-file-ignores]` may strip `T201` (or its family `T20`) from any `src/` pattern. **Evidence:** `pyproject.toml` `[tool.ruff]` lines added below `[tool.hatch.build.targets.sdist]`; `select = ["E","F","I","B","UP","T201"]`; no `ignore`/`extend-ignore`/`per-file-ignores` keys. Verified by `tests/unit/test_toolchain_config.py::test_ruff_targets_py311_with_line_length_and_effective_print_ban`.
- [x] AC-2: `pyproject.toml` contains `[tool.mypy]` with `python_version = "3.11"`, `strict = true`, **`warn_unreachable = true`** (this is *not* enabled by `strict`; it must be set explicitly), plus a `[[tool.mypy.overrides]]` block whose `module` field matches a `tests`-rooted pattern (`"tests"`, `"tests.*"`, or a list containing one of those) and which sets **both** `disallow_untyped_defs = false` AND `disallow_untyped_decorators = false`. **Evidence:** `[tool.mypy]` and `[[tool.mypy.overrides]]` (`module = "tests.*"`) in `pyproject.toml`. Verified by `tests/unit/test_toolchain_config.py::test_mypy_strict_with_warn_unreachable_and_tests_override_relaxed`.
- [x] AC-3: `pyproject.toml` contains `[tool.pytest.ini_options]` declaring `asyncio_mode = "auto"`, `testpaths = ["tests"]`, and `addopts` containing the tokens `--cov=src/codegenie`, `--cov-branch`, and `--cov-fail-under=85`. **Evidence:** `[tool.pytest.ini_options]` in `pyproject.toml`; verified by `tests/unit/test_toolchain_config.py::test_pytest_runs_under_asyncio_auto_with_coverage_gate`.
- [x] AC-4: `[tool.coverage.report]` declares `omit == ["src/codegenie/cli.py"]` **exactly**. **Evidence:** `[tool.coverage.report]` in `pyproject.toml`; verified by `tests/unit/test_toolchain_config.py::test_coverage_excludes_only_cli_py_per_phase_arch_design`.
- [x] AC-5: The TDD test file `tests/unit/test_toolchain_config.py` exists, and `pytest -q tests/unit/test_toolchain_config.py` exits 0. **Evidence:** `tests/unit/test_toolchain_config.py` exists; `pytest -q tests/unit/test_toolchain_config.py --no-cov` → `6 passed`.
- [x] AC-6: `ruff check .` and `ruff format --check .` exit 0 on the current tree. **Evidence:** `ruff check .` → `All checks passed!` (exit 0); `ruff format --check .` → `5 files already formatted` (exit 0).
- [x] AC-7: `mypy --strict src/` exits 0 on the current tree. **Evidence:** `mypy --strict src/` → `Success: no issues found in 3 source files` (exit 0).
- [x] AC-8: `[tool.coverage.run]` declares `branch = true` AND `source = ["src/codegenie"]`. **Evidence:** `[tool.coverage.run]` in `pyproject.toml`; verified by `tests/unit/test_toolchain_config.py::test_coverage_run_collects_branch_and_sources_only_src_codegenie`.
- [x] AC-9: Running `ruff check` against a Python file under `src/codegenie/` containing `print('canary')` exits non-zero and emits a `T201` diagnostic. **Evidence:** `tests/unit/test_toolchain_config.py::test_ruff_check_rejects_print_in_src_per_phase_arch_logging_strategy` — subprocess invocation of `ruff check` against an in-tree canary file under `src/codegenie/` asserts non-zero exit and `"T201"` in combined stdout/stderr; passes.
