...
- **Idempotence.** Repeatedly safe operations: `codegenie gather` (atomic `os.replace` is idempotent on identical content); `CacheStore.put` (idempotent: same key + same output is a no-op write); the `.gitignore` append (idempotent: checks for `.codegenie/` substring before appending). `codegenie audit verify` is pure-read. Not idempotent (and not required to be): `cache gc`, the audit-record write itself (one file per run by design).
