...
| 5 | chromadb writer contention under concurrent harvest | Two workflows finish validate near-simultaneously | `asyncio.Lock` around `store.add` | Second write awaits; both records land deterministically (manifest records in **insertion order** under the lock — S4-04 Notes §4 superseded the earlier "sorted by `created_at`" reading; ordering is lock-acquisition-deterministic but **content** is identical across orderings; pinned by S4-08's `asyncio.gather` test asserting `chain_head ∈ {head_ab, head_ba}`). Phase 11 pgvector swap is the resolution at portfolio scale. |
