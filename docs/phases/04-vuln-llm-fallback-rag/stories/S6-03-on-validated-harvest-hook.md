...
**Status:** Done — GREEN 2026-05-26 (phase-story-executor; Attempt #2 closed deferrals; see [`_attempts/S6-03.md`](_attempts/S6-03.md)). FallbackTier.on_validated body shipped with the **seven-step** dispatch (eligibility → gate → **idempotence pre-check** → mint → project → ingest → emit); ConfidenceGate ctor-injected; PostValidationContext kw-only carrier; HarvestSkipped event registered with closed-set reason/outcome_kind Literals; import-linter contract extended (codegenie.fallback.tier → codegenie.rag._capability_mint admitted). `SolvedExampleStore` Protocol admits sixth method `contains(sid: SolvedExampleId) -> bool` (S6-03 idempotence amendment); `ChromaPersistentStore` implements it as in-memory `_record_ids` membership check. 9 on_validated tests + 14 prep tests + **4 idempotence tests (AC-8)** + **2 mutex-property tests (AC-15)** + Protocol fence updates = **AC-1..AC-15 ALL CLOSED**. mypy --strict + ruff + lint-imports (12/12 KEPT) clean; 264 tests across the S6-03 + rag surface green.
**Effort:** M
