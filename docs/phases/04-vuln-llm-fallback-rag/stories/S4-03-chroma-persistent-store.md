...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; see [`_attempts/S4-03.md`](_attempts/S4-03.md) for the per-AC evidence table + gate log — `SolvedExampleStore` `@runtime_checkable` Protocol (4 members, async/sync split) + `SolvedExampleWriteCapability` `@final` frozen dataclass marker + `ChromaPersistentStore` adapter land at `src/codegenie/rag/store.py` (~430 lines: chromadb `PersistentClient` embedded mode + per-`(task_class, language, build_system)` collection partition via `get_or_create_collection(metadata={"hnsw:space": "cosine"})` + `asyncio.Lock` 30 s timeout → `StoreWriteContention(workflow_id)` + `asyncio.to_thread` around the sync chromadb calls + `_existing_collection`-then-`RagMiss` short-circuit on the read path + flat-dict metadata with `example_json` rehydration channel + within-process `digest()` deferred-to-S4-04 caveat encoded in docstring). Three new exceptions land at `src/codegenie/rag/errors.py` (`StoreWriteContention` carries typed `workflow_id: WorkflowId`; `StoreClosed` + `StoreCorrupted` are markers; `StoreCorrupted` is declared-not-raised per Implementation Outline §2). Story-scoped gates green: 20 story tests (12 unit + 1 integration + 7 fence sub-tests), 472 fence tests, `mypy --strict src/` (231 files), `ruff check`/`ruff format --check`, `make lint-imports` (11 contracts kept / 0 broken — one new `ignore_imports = ["codegenie.rag.store -> chromadb"]` edge appended to the ADR-0003 chromadb contract per the in-file placeholder comment), pre-commit `run --all-files` clean after `chromadb>=0.5,<1.0` joined the mypy hook's `additional_dependencies`. Full suite 6943 passed / 42 skipped / 4 deselected / 9 xfailed — the pre-existing L-2 `tsconfig_pathological` macOS timing flake recurred; CI Linux clean. Three new lessons (L-S403-1 chromadb `to_thread` invariant `list[...]` types, L-S403-2 pre-commit mypy `additional_dependencies` must mirror every path-scoped admission, L-S403-3 chromadb 0.6.3 posthog telemetry stderr noise) captured.
**Effort:** L
**Depends on:** S1-04 (`SolvedExample` with `embedding_vector: EmbeddingVector` per the 2026-05-24 post-Done amendment, `Query`, `RetrievalOutcome`, `RecordProvenance` Pydantic models; `SolvedExampleId`, `StoreDigest`, `Similarity`, `EmbeddingVector` Newtypes), S1-05 (path-scoped fence admits `chromadb` only under `src/codegenie/rag/`; creates the empty `src/codegenie/rag/__init__.py` namespace marker), S4-01 (creates `src/codegenie/rag/errors.py` and the `rag` package's first error types — **this story *extends* that module**)
...
**Cross-story blocker (note B) — RESOLVED 2026-05-24 by S1-04 post-Done amendment.** AC-4 reads `example.embedding_vector` and `add()` passes it to chromadb as `embeddings=[...]`. ADR-0016 §Consequences mandates that canonical records carry "embedding model digest **+ vector**" so `codegenie rag rebuild` can re-insert into chromadb *without re-embedding*. S1-04's original `SolvedExample` model shipped without that field; chromadb's default embedding function is `all-MiniLM`, not the pinned `fastembed` BGE-small, so the store *must* receive an explicit pre-computed vector — it cannot fall back to letting chromadb embed `documents`. **Resolution applied:** S1-04 was amended in `src/codegenie/rag/models.py` to add `embedding_vector: EmbeddingVector` (a one-field extension-by-addition — sanctioned per CLAUDE.md). 55 rag-model + roundtrip tests green; `mypy --strict src/codegenie/rag/` clean. S4-03 is now unblocked.
