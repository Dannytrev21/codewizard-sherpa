...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; the S1-07 BLOCKED-PARTIAL precondition is resolved — S1-07 landed earlier this session as `Phase-4 S1-07 GREEN — kernel-frozen Phase-3 baseline extension`). `tests/fence/test_phase4_diff_within_allow_list.py` shipped with the canonical `ALLOWED_PREFIXES` + `ALLOWED_EXACT` allow-list buckets per AC-5. 14 tests cover AC-5 live gate, AC-6 fail-loud diagnostic, AC-7 master-skip, AC-8 vacuous-allow-list invariants, AC-9 planted-violation table over 12 inside/outside paths. AC-1 phase-frozen passes (Phase-2 + Phase-3 baselines both 18/18 green in `tests/fence/test_kernel_frozen.py`).
**Effort:** S
