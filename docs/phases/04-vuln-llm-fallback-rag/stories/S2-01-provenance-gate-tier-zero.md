...
**Status:** Done — GREEN 2026-05-24 (phase-story-executor; see [`_attempts/S2-01.md`](_attempts/S2-01.md) for the per-AC evidence table + gate log — `ProvenanceGate` tier-0 short-circuit landed at `src/codegenie/fallback/provenance_gate.py` (165 lines: `@runtime_checkable` `ProvenanceClassifier` Protocol facade, `_APP_LAYER_PROVENANCE_KINDS = frozenset({"app_direct","app_transitive","app_vendored","both"})`, pure `is_app_layer` Specification-pattern predicate, `@dataclass(frozen=True, slots=True)` `ProvenanceGate` with the two-field state `{classifier, event_log}`). `ProvenanceClassified` joined `WorkflowInternalEvent` as variant #17. Three fence tests pin the zero-spend boundary: AST imports, signature parameter set, and dataclass field set. Story-scoped gates green: 65 story tests (26 unit-gate + 39 events including 4 new), 410 fence tests, `mypy --strict src/` (214 files), `ruff check` + `ruff format --check`, `lint-imports` (10 contracts kept / 0 broken). Two known pre-existing local-env failures (`tsconfig_pathological` timing flake, `lint_imports_canary` PATH issue) are documented in the attempt log and not caused by this story.)
**Effort:** S
...
- [x] **AC-9 — Event-kind registration cannot silently drift.** `tests/unit/plugins/test_events.py` (or a new `tests/fence/test_event_kinds_complete.py` if the suite already has that convention by implementation time) asserts `TypeAdapter(WorkflowInternalEvent).json_schema()["discriminator"]["mapping"]` contains `"provenance_classified"` and that `ProvenanceClassified` constructs with typed payload fields. A typo such as `"provenanceClassified"` or class-not-in-union fails.
- [x] **AC-10 — Strict typing and lint.** `ruff check`, `ruff format --check`, and `mypy --strict` are clean for `src/codegenie/fallback/provenance_gate.py`, the touched event module, and the new tests. Public signatures contain no `Any`, and no new `# type: ignore` is introduced outside test-only mypy-negative snippets.
