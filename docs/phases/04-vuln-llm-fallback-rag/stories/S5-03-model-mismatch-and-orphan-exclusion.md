...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; see [`_attempts/S5-03.md`](_attempts/S5-03.md)). 16 tests cover AC-1 (module shape), AC-2 (filter signature byte-identical to retriever hook), AC-3 (one event per exclusion with current_model + sample_stale_model), AC-4 (zero-exclusion silence), AC-5 (partial-mismatch retriever integration), AC-6 (all-mismatch → bare RagMiss + reason event + filter event), AC-7 (determinism), AC-9 (chain-orphan exclusion runs first → no double attribution), AC-10 (RagRecordModelMismatch event extended with current_model + sample_stale_model + count >= 1 validator), AC-12 (once-per-call invariant). Event emission moved from the retriever into the filter (it owns the digest knowledge).
**Effort:** S
