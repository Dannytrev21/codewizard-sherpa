...
**Status:** Done — 2026-05-24 (phase-story-executor; see [`_attempts/S1-02.md`](_attempts/S1-02.md) for the per-AC evidence table + gate log — Phase-4 `PlanProposal` closed Pydantic-v2 discriminated union landed with four variants, two co-resident `Annotated[str, AfterValidator]` smart-constructor newtypes (`UnifiedDiff` 64 KB cap / `SandboxedRelativePath` path-escape), the cross-field `diff paths ⊆ files` `@model_validator`, and 45 story-scoped tests covering happy/sad-paths with distinctive-keyword assertions per F9. Story-scoped gates green: 45 tests, `mypy --strict src/codegenie/fallback/`, `ruff`, `ruff format --check`, plus the pre-existing `tests/fence/test_phase4_no_raw_str_for_domain_ids.py` confirming no raw-`str` domain annotations leaked under `fallback/`.)
**Effort:** M
**Depends on:** S1-01
...
- [x] AC-5 — `tests/unit/fallback/test_plan_proposal.py` covers happy + sad paths for every variant. Every sad-path `UnifiedDiff`/`SandboxedRelativePath` test asserts a **distinctive keyword in the `ValidationError`** (F9) — not bare `pytest.raises(ValidationError)` — so a rejection for the wrong reason fails the test:
...
- [x] AC-6 — **Schema-totality property** (`tests/property/test_plan_proposal_schema_totality.py`). The schema is produced via `TypeAdapter(PlanProposal).json_schema()` — `PlanProposal` is an `Annotated[...]` alias with no `.model_json_schema()` method (F13):
...
- [x] AC-7 — **`assert_never` exhaustiveness via subprocess mypy** (`tests/property/test_plan_proposal_match_exhaustive.py`):
...
- [x] AC-8 — `model_construct` is forbidden in production code under `src/codegenie/fallback/` and `src/codegenie/rag/`. Test `tests/fence/test_phase4_no_model_construct.py` AST-walks both directories (handling `not yet existent`) and asserts no `*.model_construct(` callsite. Skeleton lands here; coverage grows as later stories add code.
- [x] AC-9 — **Rationale-discipline AST guard** (`tests/fence/test_no_rationale_in_prompts.py`): walks `src/codegenie/fallback/` and asserts `PlanProposal*.rationale` is **never** read into a string that flows into `prompt_builder.build(...)` (no `f"... {plan.rationale} ..."` patterns under `fallback/`). Skeleton lands here (S2-04 will exercise it).
- [x] AC-10 — `PlanProposal` is exported from `src/codegenie/fallback/__init__.py`; the four variant classes are also exported individually.
- [x] AC-11 — `mypy --strict src/codegenie/fallback/` clean. `ruff check`, `ruff format --check` clean. The TDD plan's red test exists, was committed, and is green.
- [x] AC-12 — **`SandboxedRelativePath` is defined by this story** (F1 — it does not exist in the codebase; `phase-arch-design.md §Data model` and S1-01's out-of-scope wrongly call it "Phase-3-owned"). It is `Annotated[str, AfterValidator(_validate_sandboxed_relative_path)]` in `src/codegenie/fallback/plan_proposal.py` — a sibling of `UnifiedDiff`, for the identical reason (`UnifiedDiff` is deliberately not a `NewType` because a validator must fire on every Pydantic construction; `manifest_path` smart-construction has the same requirement, and the LLM emits it as a raw JSON string at the SDK boundary). It is **not** added to `codegenie.types.identifiers` (that would touch the fenced `__all__` surface and require the S1-01-style cross-file reconciliation; a Pydantic-validated `Annotated` type cannot be a bare `NewType` anyway). The pure helper `_validate_sandboxed_relative_path(value: str) -> str` rejects, each with a distinct message:
