...
**Status:** Done — GREEN 2026-05-24 (phase-story-executor; see [`_attempts/S2-03.md`](_attempts/S2-03.md) for the per-AC evidence table + gate log — `CanaryGuard` + `scan_pure` + `_validate_patterns` + a 50-row curated `INJECTION_PATTERNS` corpus (all 15 AC-2-mandated IDs present) land at `src/codegenie/fallback/fence/canary.py`. Six new test files cover: AC-2 corpus invariants + 7 dedicated rejection paths (under-50, duplicate-id, duplicate-bytes, bad-id-shape, empty-bytes, non-UTF-8, substring-shadowing); AC-3 AST-allowlist purity + signature pin + empty-payload row; AC-4 `scan` classmethod on class & instance; AC-5 `isinstance(CanaryGuard(), Scanner) is True`; AC-6 Hypothesis property (500 examples) asserting exact `pattern_id` past the 16 KB cap; AC-7 end-to-end adversarial proof through `FenceWrapper` for 5 patterns with `b"BENIGN " * 3000 = 21_000 bytes` filler past the cap + exactly one `CanaryCollisionEvent` via `EventLog.replay()`; AC-8 60-row curated corpus with all-15-mandated-IDs-covered guard; AC-9 25-row clean corpus with deliberate near-misses; AC-10 nonce-collision detection (priority over pattern match). Gates green: 119 story-scoped tests, 413 fence tests, `mypy --strict src/` (217 files), `ruff check` + `ruff format --check`, `lint-imports` (10 contracts kept / 0 broken).)
**Effort:** M
