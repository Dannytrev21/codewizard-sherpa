...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; see [`_attempts/S3-04.md`](_attempts/S3-04.md) for the per-AC evidence table + gate log — `CassetteSanitizer` lands at `src/codegenie/fallback/cassette/sanitizer.py` (~470 lines: bytes-typed `_BODY_SECRET_PATTERNS` catalog, 5 pure helpers under the AC-23 AST fence, `verify_cassette` as the sole I/O surface, `CassetteVerification.passed` derived property making silent-pass leaks structurally unrepresentable, `Violation` kind/field coupling validator). `tests/conftest.py` extended with the `vcr_config` fixture wiring the ADR-0014 hooks + `CODEGENIE_LIVE_LLM` switch (S3-03's `egress_test_loopback` fixture preserved per Rule 7 — story said "create" but file already existed). 155 story-scoped tests pass: 38 unit (incl. Hypothesis idempotence biased to redaction path with `@example` per pattern, 200 examples × 2 directions), 62 sanitizer-corpus (31-row `CORPUS` × req/resp), 30 innocuous-corpus, 14 AST-purity fence (planted-positives × 9), 5 Makefile-discipline fence (parametrized planted-positives), 6 real-`vcr.request.Request` integration. Gates green: 457 fence tests, 6795 unit+integration (L-2 / L-4 documented local-env flakes deselected; CI Linux clean), `mypy --strict src/` (225 files), `ruff check` + `ruff format --check`, `lint-imports` (11 contracts kept / 0 broken — `codegenie.fallback.cassette` added to the ADR-0010 BudgetToken `source_modules`), `uv.lock` regenerated for the `pytest-recording` dev-dep. L-10 captured: `isinstance(x, dict)` fails on third-party `MutableMapping`-only subclasses (vcrpy's `HeadersDict`); the real-type integration test caught it; dict-shim unit tests did not.)
