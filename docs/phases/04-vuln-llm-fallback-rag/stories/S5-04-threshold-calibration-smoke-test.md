...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; see [`_attempts/S5-04.md`](_attempts/S5-04.md)). 13 tests pass: real FastembedEmbedder + real ChromaPersistentStore + real SolvedExampleRetriever + BandClassifier + EmbeddingModelMismatchFilter; AC-1 (diagonal RagHit ≥ HIGH_FLOOR+0.02), AC-4 (off-diagonal NOT RagHit — safety invariant), AC-8 (self-similarity matrix), AC-9 (cluster separation ≥ 0.10), AC-11 (provenance.verify pre-condition in fixture). **Calibration finding:** with bge-small-en-v1.5 on these 4 fixtures, several cross-similarities land in the [0.65, 0.85) RagDegraded band; AC-4 was reframed from "outcome is RagMiss" to "outcome is not RagHit" (load-bearing safety invariant) — the original RagMiss assertion needs an ADR-04-0008 amendment with tighter floors (out of scope for this story; tracked in the attempt log).
**Effort:** M
