...
**Status:** Done — GREEN 2026-05-24 (phase-story-executor; see [`_attempts/S2-04.md`](_attempts/S2-04.md) for the per-AC evidence table + gate log — `PromptBuilder` + `TrustedPrompt` + `FencedPromptBody` newtypes land at `src/codegenie/fallback/fence/prompt_builder.py` (~200 lines: two `NewType(..., str)` co-located beside their sole smart constructor, pure `_ordered_untrusted_segments` ordering helper, `@dataclass(frozen=True, slots=True)` `PromptBuilder` whose `build(...)` is a composition shell over `FenceWrapper.fence(...)`). `PromptAssembled` + `SegmentCountTruncated` join `WorkflowInternalEvent` as variants #18 #19 (internal-variant count grew 19 → 21). Five new test files (sole-mint AST walk + positive-control violator fixture, no-bypass AST walk over 4 illegal patterns, behavior tests covering AC-4..AC-10, plus event-union coverage for both new variants) — 71 story tests green on first GREEN. Gates green: 414 fence tests, 5827 unit+integration (2 pre-existing local-env failures in `tests/unit/test_lint_imports_canary.py` documented in S2-01/02/03), `mypy --strict src/` (218 files), `ruff check` + `ruff format --check`, `lint-imports` (10 contracts kept / 0 broken).)
**Effort:** S
