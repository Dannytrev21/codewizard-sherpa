...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; see [`_attempts/S4-01.md`](_attempts/S4-01.md) for the per-AC evidence table + gate log — `Embedder` Protocol (3 members, `@runtime_checkable`) + `FastembedEmbedder` (refuse-start with 7 typed branches + L2-normalize/tuple-finalize on the return boundary) land at `src/codegenie/rag/embedder.py`. `codegenie embeddings bootstrap` CLI body at `src/codegenie/rag/cli.py` (4 exit branches: first-write / no-op / upgrade / drift; module-scope `_seam_write_lock` so idempotence is testable byte-identically). Typed errors at `src/codegenie/rag/errors.py` with `kind: Literal["model_name","sha256"]` discriminator. Click registration in `src/codegenie/cli.py` mirrors the `vuln-index` defer-import precedent. 25 unit tests + 5 fastembed-marked (real BGE-small) + 5 fence tests + AC-10 path-scoped fence + AC-11 mypy/ruff all GREEN. `lint-imports` 11 kept / 0 broken (added two `ignore_imports` edges for the admitted callsites on the ADR-0003 contract). Pre-commit mypy hook gained `fastembed` + `numpy` `additional_dependencies`. Pre-existing L-2 / L-4 local-env failures recur per S2-01..S3-06; CI Linux clean.)
**Effort:** M
