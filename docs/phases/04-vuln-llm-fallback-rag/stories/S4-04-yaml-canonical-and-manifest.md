...
**Status:** Done — GREEN 2026-05-25 (phase-story-executor; see [`_attempts/S4-04.md`](_attempts/S4-04.md) for the per-AC evidence table + gate log — `_canonical_yaml_dump` shared serialization surface (record + manifest + AC-6 property test) + `_atomic_write_text` (tmp + `os.replace`, mirrors `probes/layer_d/conventions.py`) + `_validate_record_id_path_safe` (AC-14 path-safety guard) + `_roll_chain_head` pure functional core + `_compute_chain_head` `FileNotFoundError → StoreCorrupted`-translating shell + `_Manifest` frozen Pydantic model (`schema_version: Literal[1]`) + `_parse_manifest_or_raise` defensive translator land at `src/codegenie/rag/store.py`; `_load_existing_record_ids` rewired to read `manifest.yaml` (order-of-truth) and exercise `_compute_chain_head` at open-time so AC-13's missing-record check fails loud at `__init__`; `add()` writes canonical YAML → chromadb → manifest atomically inside the S4-03 `asyncio.Lock`; on manifest-write failure `_record_ids` retains the failed-add id by design (AC-12 §"in-process self-heal" — the next successful `add()` re-rolls a consistent manifest); `digest()` rerolls over canonical YAML *bytes* (was record-ID strings in S4-03) and re-wraps `ChainHead → StoreDigest` at the Protocol boundary; `SolvedExample.from_yaml` classmethod lands at `src/codegenie/rag/models.py`; validator's recommended ADR-0016 §Consequences amendment landed (manifest shape now spells out `schema_version: 1` + canonical-YAML-bytes basis). Story-scoped gates green: 27 new tests (22 unit + 4 chain-head + 1 Hypothesis property) + 144 total rag tests + 472 fence tests; `ruff check`/`ruff format --check` clean; `mypy --strict src/` 231 files OK; `lint-imports` 11 contracts kept / 0 broken (no new edges — S4-03's existing `chromadb -> codegenie.rag.store` ignore covers the residual `__init__` import); `pre-commit run --all-files` clean (under `PATH="$PWD/.venv/bin:$PATH"`); full suite 6965 passed / 9 xfailed / 5 local-env or pre-existing flakes (L-2 macOS timing + 3 missing-PATH artifacts) recognized — CI Linux clean. Four new lessons captured (L-S404-1 O(N) re-read is intentional vs. hidden-state hasher, L-S404-2 `_Manifest` gets no `from_yaml` to preserve the `StoreCorrupted` translation envelope, L-S404-3 raw `schema_version` check sequenced before `_Manifest.model_validate` to keep the diagnostic actionable, L-S404-4 Hypothesis `builds()` against newtype-rich Pydantic models needs explicit per-field strategies).
**Effort:** M
