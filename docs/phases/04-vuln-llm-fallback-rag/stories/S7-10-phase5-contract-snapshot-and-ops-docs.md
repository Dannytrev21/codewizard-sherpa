...
**Status:** BLOCKED-PARTIAL — 2026-05-27 (phase-story-executor Attempts #1/#2/#3/#4; **17 of 18 ACs Done**; only AC-18 remains, BLOCKED on S6-06 GREEN-complete; see [`_attempts/S7-10.md`](_attempts/S7-10.md)). Attempt #4 closed AC-5 (all 6 mutation guards now shipped — `running_total` return-type-narrowing + `cassettes.lock` format pinning) + AC-8 (module docstring extended to name Phase 4 as the additive-captures source + cite ADRs 0002/0009/0010/0013/0014). Earlier — Attempt #3 closed AC-1/2/3/4/6/7/17 via the five Phase-4 captures landing on the freshly-shipped contract-snapshot scaffold (`939df6d`); Attempt #2 closed AC-9/10/15 (fixture slice); Attempt #1 closed AC-11/12/13/14/16 (ops-docs slice). **Only AC-18 remains** (registry-count fence — depends on Phase-3 S6-06 GREEN-complete shipping the `@register_snapshot_kind` + `@register_delta_rule` registries). 13 contract-snapshot tests green.. **AC-9 + AC-10 + AC-15 closed in Attempt #2**: shipped `tests/fixtures/fallback_tier_callable.py` (`@runtime_checkable` `FallbackTierCallable` Protocol + module-level instance + import-time isinstance assertion; **Rule-7 surface**: story spec references nonexistent `RecipeApplication`, shipped contract is `PlanOutcome` per ADR-04-0004), 6-test behavior suite verifying asyncio.run + typed PlanOutcome end-to-end (placeholder dispatch returns `Refused` today; leaf-invoked branch deferred with S6-01 GREEN-complete), and `mkdocs build --strict` runs clean. **Attempt #1 (2026-05-26)** had shipped the ops-docs slice (`secrets.md` NEW + `cassettes.md`/`embeddings.md` extended + `tests/integration/test_ops_docs_exist.py`) closing AC-11/12/13/14/16. **Superseded note (2026-07-25 CI-health pass):** an earlier sentence here claimed the contract-snapshot artifacts were "absent on master" — that is stale and has been removed. `tests/integration/test_phase5_contract_snapshot.py`, `tests/golden/phase5-contract/snapshot.json`, and `scripts/regen_probe_contract_snapshot.py` all exist on master and are green. The single remaining blocker is **AC-18**: `grep -rn 'register_snapshot_kind\|register_delta_rule' src/` returns **0 hits**, i.e. Phase-3 S6-06 shipped the *minimal inline-dispatch scaffold* and not the `@register_snapshot_kind` / `@register_delta_rule` registries the registry-count fence measures. Status is therefore BLOCKED-PARTIAL, not GREEN-partial: **do not execute further Phase-4 Step-7 stories until S6-06 ships the registries and AC-18 closes.**
**Effort:** M
