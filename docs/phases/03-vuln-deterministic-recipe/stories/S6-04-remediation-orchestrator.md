...
**Status:** BLOCKED — architectural gap **resolved** by [ADR-0015](../ADRs/0015-orchestrator-self-loads-repo-context-and-resolves-cve.md) (2026-05-21); awaiting a `/phase-story-validator` re-harden. The 2026-05-21 re-validation (verdict **RESCUE**) found S6-04 sat on a genuine architectural gap a validator could not close: (G1) the ADR-0001-frozen `run(repo, cve, context)` / `__init__` signatures have no slot for a `RepoContext`, and arch §Control-flow step 1 ("the CLI loads `repo-context.yaml`") structurally contradicted that frozen surface; (G2) no shipped `VulnIndex` method maps `CveId → VulnerabilityRecord`. **ADR-0015 closes both:** the orchestrator self-loads the dependency set off the `repo` path via a narrow `load_installed_dependencies` loader (there is no `RepoContext` Python type — only the `repo-context.yaml` artifact), so `run` / `__init__` stay frozen; `CveId → VulnerabilityRecord` resolves via an additive `VulnIndex.find_by_cve` intersected with the loaded dependency set through a pure `resolve_cve` resolver (zero matches → `RemediationNotApplicable`, one → proceed, many → `RequiresHumanReview`); and `BundleBuilder.build` is orchestrator-owned in the `run()` preamble, seeding `SubgraphState.bundle`. **Next step:** a `/phase-story-validator` re-harden must fold ADR-0015 + the patchable dependency-drift list (B1–B9 in the re-validation report) into the ACs, Implementation outline, TDD plan, and Files-to-touch — including the additive `SubgraphState` slots, the `repo_context.py` module, and the real (non-`empty()`) `CapabilityBundle`. Until that re-harden lands, S6-04 stays BLOCKED and no later Step-6/7/8/9 story may be executed. Full audit: [`_validation/S6-04-remediation-orchestrator.md`](_validation/S6-04-remediation-orchestrator.md) (§"Re-validation — 2026-05-21"). Executor Attempt 1: [`_attempts/S6-04.md`](_attempts/S6-04.md).
**Effort:** L
