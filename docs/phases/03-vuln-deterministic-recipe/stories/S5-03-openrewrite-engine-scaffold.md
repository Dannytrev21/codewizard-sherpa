...
**Status:** Done — GREEN 2026-05-20 (phase-story-executor; see [`_attempts/S5-03.md`](_attempts/S5-03.md) Attempt 2 for the per-AC evidence table, the as-built drift resolutions, and the gate log — `openrewrite.py` 100% branch coverage, full suite 5766 passed, mypy `--strict` + ruff + import-linter + pre-commit all green). The prior BLOCKED `apply`-2-tuple-vs-Protocol contradiction was resolved by [ADR-0014](../ADRs/0014-recipe-engine-surfaces-transform-via-transform-registry.md) + S5-01b (`TransformRegistry`, GREEN): `apply` returns a bare `RecipeOutcome`; the produced `DockerfileBaseImageTransform` is surfaced via a constructor-injected `TransformRegistry`. Nine as-built drift resolutions are logged in the attempt log (chiefly: error taxonomy is **6** ids not 5 — `recipe.jail_setup_failed` is forced by the as-built six-variant `JailedSubprocessResult`, mirroring S5-02; AC-Conf-1 / AC-CI-2 retargeted to the real test paths; Outline §12's `transforms/__init__.py` re-export withdrawn to mirror S5-02). **AC-Phase7-1** is environment-gated/deferred: the integration-test infrastructure (marker, skip guards, fixture, `expected.diff` golden) ships, but a green real-JVM run needs Phase 7's authored recipe + a provisioned `rewrite-cli.jar` — the exact shape of S5-02's AC-Gold-1. The **Re-execution note** below remains authoritative for the contract surface.
**Effort:** M
...
| ~~`src/codegenie/transforms/__init__.py`~~ | **NOT touched (executor 2026-05-20).** Outline §12's re-export is withdrawn: S5-02 (GREEN) deliberately did **not** re-export `NpmLockfileRecipeEngine` from `transforms/__init__.py` — that module is the ADR-0001 Phase-5 *contract-surface* home, not an engine registry, and `engines/__init__.py` ships "no public re-exports". Notes §2 mandates mirroring S5-02 literally. Engines are imported directly (`from codegenie.transforms.engines.openrewrite import …`), exactly like `npm_lockfile`. AC-Tool-4's fence still passes — zero `src/` importers to filter. |
...
| `tests/unit/transforms/test_recipe_engine_protocol.py` | **Amend** — S5-01 (GREEN) created it at this `tests/unit/transforms/` path (NOT `tests/integration/` — the original AC-Conf-1 path was drift, flagged by Attempt-1's log). This story adds `test_openrewrite_engine_satisfies_protocol` + `test_openrewrite_engine_apply_return_annotation` cases. Pre-existing S5-01 cases preserved |
...
| `tests/unit/exec/test_allowed_binaries.py` | **Amend** — append `test_java_not_in_allowed_binaries_phase3` case to the as-built allowed-binaries suite (the real path; `tests/fence/test_allowed_binaries_invariants.py` never existed — Rule 7: one location). Phase 7 deletes the case |
...
| `docs/phases/03-vuln-deterministic-recipe/ADRs/0006-hexagonal-subprocessjail-port-bwrap-sandbox-exec.md` | **Amend** — 2026-05-20 Amendment block: `JvmEnv` added to `JailedEnv` discriminated sum (additive widening) |
| `tests/unit/transforms/test_sandbox_jail.py` | **Amend (executor 2026-05-20, not in original list)** — `test_module_exports_exact` pins the `sandbox_jail` `__all__` by exact set equality; the story-mandated additive `JvmEnv` export (Outline §12) requires adding `"JvmEnv"` to the expected set |
