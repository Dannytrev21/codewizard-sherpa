...
- [ ] **AC-12** A unit test `test_this_is_the_terminal_byte_edit_allowlist` globs `tests/fence/` for byte-edit-allowlist fence files (pattern `*no_byte_edits*`) and asserts **exactly one** exists — this file. Per [production ADR-0043](../../../production/adrs/0043-extension-by-addition-means-no-silent-edits.md), Phase 7's allowlist is the *last* per-phase enumerated allowlist: no future phase adds a `_PHASE8_BYTE_EDIT_ALLOWLIST` symbol or a `test_phase8_no_byte_edits_*.py` fence. A planted second allowlist fence file fails this guard. The test docstring cites ADR-0043 and states that new frozen surfaces use a contract + snapshot test (the probe-ABC pattern), not allowlist rows.
...
- **Phase 8+ allowlist rows — there are none.** Per [production ADR-0043](../../../production/adrs/0043-extension-by-addition-means-no-silent-edits.md), Phase 7's allowlist is the *last* per-phase enumerated allowlist; future phases add no rows and no new per-phase allowlist fence. `_PHASE7_BYTE_EDIT_ALLOWLIST` is **terminal** — do NOT design it for a `_PHASE8_BYTE_EDIT_ALLOWLIST` successor. New frozen surfaces in later phases use a contract + snapshot test (the probe-ABC pattern), not allowlist rows. AC-12 enforces this.
...
- **Terminal allowlist (production ADR-0043):** `_PHASE7_BYTE_EDIT_ALLOWLIST` is the *last* per-phase enumerated allowlist. Do NOT shape the test layout for a `_PHASE8_BYTE_EDIT_ALLOWLIST` successor and do NOT add a phase-extension seam — ADR-0043 makes that a non-goal. AC-12's guard fails if a second allowlist fence appears. Later phases protect new frozen surfaces with a contract + snapshot test (the probe-ABC pattern: a `*.v1.json` snapshot diffed in CI), never another allowlist.
