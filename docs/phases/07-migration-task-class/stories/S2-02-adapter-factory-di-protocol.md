...
- [ ] **AC-10 — TDD red test exists, committed, green.** `tests/unit/primitives/vuln_provenance/test_factory.py::test_default_factory_passes_only_declared_di_kwargs` was the first failing test; impl makes it green.
- [ ] **AC-11 — Lint / type clean.** `ruff check`, `ruff format --check`, `mypy --strict` clean on `src/codegenie/primitives/vuln_provenance/factory.py` + the test module + the conftest. `make lint-imports` green.
