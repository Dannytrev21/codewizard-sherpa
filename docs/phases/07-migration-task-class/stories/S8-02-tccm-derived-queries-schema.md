...
> **⚠ Amendment A sequencing note (2026-05-20).** Phase 7 Amendment A ([`../final-design.md` §Amendment A](../final-design.md)) adds six gather probes (Steps 13–15). The plugin's `tccm.yaml` `must_read`/`should_read` bands must reference the new probe slices; coordinate with [S13-03](S13-03-amendment-a-schemas-and-fence.md) and [S15-01](S15-01-runtime-shell-invocation-probe.md)..[S15-03](S15-03-runtime-compat-probe.md). See [`README.md` §"Stories — Amendment A"](README.md).
