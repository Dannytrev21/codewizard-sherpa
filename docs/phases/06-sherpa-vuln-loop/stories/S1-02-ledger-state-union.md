...
- [ ] **AC-4 — `TransitionEvent` model carries the five fields `final-design.md §"State model"` mandates.** A single frozen Pydantic model:
  ...
  class TransitionEvent(BaseModel):
      ...
      triggering_outcome: JsonValue                         # see Notes — typed-union deferred to producer side
...
- [ ] **AC-8 — Chain-head pure helper substrate (ADR-0003 replay-verification gate).** A pure helper `_compute_chain_head` lives in `src/codegenie/workflows/_chain.py`:
  ...
  def _compute_chain_head(prior_head: ChainHead, event: TransitionEvent) -> ChainHead:
      """Pure: BLAKE3(prior_head_bytes || canonical_event_bytes); returns ChainHead (bare 64-hex, no prefix — matches the Phase-4 newtype shape)."""
  ```
...
- Implement `_compute_chain_head` as a *pure* helper in `_chain.py`: take `prior_head` + `event`, serialize the event via `event.model_dump_json().encode()` (Pydantic emits sorted JSON), separate with `RECORD_SEP (\x1e)` to defuse boundary-shift collisions, feed `prior_head.encode() + RECORD_SEP + event_bytes` to BLAKE3, return `ChainHead(<bare 64-hex>)`. The existing `ChainHead` newtype (Phase-4 S4-04) is bare 64-hex without any `"blake3:"` prefix — do NOT prefix on construction; route the BLAKE3 call through `codegenie.hashing.content_hash_bytes` (ADR-0001 chokepoint) and strip any prefix from its return. No I/O, no env, no clock — the AST fence enforces this.
...
- **"Terminal" has two definitions — keep them straight.** *Class-level terminal* (used by AC-6 and by S1-01's `TerminalState` Literal) is the three-kind set `{completed, awaiting_human_review, failed_unrecoverable}` — a contract declaration that consumers (Phase-6.5 scorecards, Phase-9 worker) read as "the run reached a class-final state." *Operationally terminal* (used by AC-5 §3 "zero outgoing edges") is the two-kind set `{completed, failed_unrecoverable}` — a graph-level property of `_LEGAL_TRANSITIONS`. `awaiting_human_review` is class-level terminal AND operationally resumable (legal exits: `→ plan_ready`, `→ completed`, `→ failed_unrecoverable`). Tests that conflate the two will trip on the resume edges; the test split landed under S1-02 (`test_ac5_operationally_terminal_states_have_zero_outgoing_edges` vs `test_ac6_terminal_partition_byte_equal...`) pins both definitions and is the reference template for downstream stories.

- **`triggering_outcome: JsonValue`, not a typed union — and why.** The original story prose enumerated `RecipeOutcome | NodeTransition | GateOutcome`. The S1-02 attempt log recorded three concrete blockers to landing that union here: (1) `NodeTransition.Advance.state: SubgraphState` forces a `model_rebuild()` that couples `vuln_ledger.py` to `codegenie.plugins.subgraph` (out of Phase-6 scope and closes a kernel cycle); (2) there is no `GateOutcome` type in the codebase — the closest existing type is `TrustOutcome` (a single class, not a sum type); (3) the substrate only needs deterministic JSON bytes for the chain-head — the typed shape lives naturally on the producer side. Resolution: this story stores `triggering_outcome: JsonValue` and the typed-union responsibility moves to S3-01 (plugin subgraph), which serializes via `outcome.model_dump(mode='json')` before constructing the `TransitionEvent`. If a future story argues for re-introducing the typed union here, it must first land the `SubgraphState` decoupling AND introduce a real `GateOutcome` sum type.
