...
- **Fix:** Set `__test__ = False` on the class **after the class body**, not inside it. Putting it inside a `Protocol` body adds it to `_get_protocol_attrs` so stubs would have to declare it; setting it after the body keeps the attribute out of the protocol's contract.
- **Why it matters:** Any future domain type that contains the word `Test` (`TestInventoryAdapter`, `TestId` was OK because it's a `NewType` alias of `str`, but future `TestMatrix`, `TestSuiteId`, etc. would hit this) needs the same treatment.
