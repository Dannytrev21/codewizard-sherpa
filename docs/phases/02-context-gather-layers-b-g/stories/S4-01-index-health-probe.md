...
**Status:** Ready · VALIDATED (HARDENED — see `_validation/S4-01-index-health-probe.md`)
**Effort:** L
...
- [ ] **AC-3 — `runs_last=True` registry annotation is respected by the coordinator.** Two checks:
...
- [ ] **AC-4 — Registry-loop dispatcher (Gap 3 — Open/Closed at the file boundary).** `async def run(self, repo: RepoSnapshot, ctx: ProbeContext) -> ProbeOutput`:
...
- [ ] **AC-10 — Slice shape per `localv2.md §5.2 B2` + outer-key invariant.** Output slice is `{"index_health": {<index_name>: {"freshness": <IndexFreshness JSON via model_dump(mode="json")>, "confidence": "high"|"medium"|"low", "last_indexed_at": <iso8601 str | None>, "current_commit": <sha str>}, ...}}`. The typed `IndexFreshness` value is included as `"freshness"` (nested object with `"kind"` discriminator); the flat `"confidence"` string is derived per AC-9 (backward compat with `localv2.md` slice shape). `"last_indexed_at"` is the ISO8601 string from `Fresh.indexed_at.isoformat()` only when the variant is `Fresh`; on every `Stale(...)` variant it is `None` (the indexed_at is no longer authoritative).
