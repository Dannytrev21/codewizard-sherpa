...
**Status:** **BLOCKED-PARTIAL (2026-05-16)** — `GeneratedCodeProbe` + `SemanticIndexMetaProbe` shipped GREEN with full test/mypy/lint coverage (see [`_attempts/S4-06.md`](_attempts/S4-06.md)). `NodeReflectionProbe` is blocked by the same precondition that put S4-04 in BLOCKED state: vendored grammar binaries under `tools/grammars/{javascript,typescript}.so` are 68-byte placeholder stubs that cannot be loaded by `tree_sitter.Language(...)`. Real grammars (~250–500 KiB Linux x86_64) must land before NodeReflection can ship.
**Effort:** M
