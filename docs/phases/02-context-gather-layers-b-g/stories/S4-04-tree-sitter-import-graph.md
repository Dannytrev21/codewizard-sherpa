...
**Status:** **GREEN (2026-05-16)** — every AC has runtime evidence; full unit suite (2297 passed) + ruff + `mypy --strict` clean. See [_attempts/S4-04.md](_attempts/S4-04.md) attempt 2 for the per-AC evidence table and the 02-ADR-0011 surface translations. Implementation at [`src/codegenie/probes/layer_b/tree_sitter_import_graph.py`](../../../../src/codegenie/probes/layer_b/tree_sitter_import_graph.py); tests at [`tests/unit/probes/layer_b/test_tree_sitter_import_graph.py`](../../../../tests/unit/probes/layer_b/test_tree_sitter_import_graph.py). Grammar load flows through `codegenie.grammars.lock.language_for(name) -> tree_sitter.Language` (PyPI wheels per 02-ADR-0011); ACs mentioning BLAKE3 / `tools/grammars.lock` / `load_and_verify` are superseded by `pip --require-hashes` at the wheel boundary.
**Effort:** M
