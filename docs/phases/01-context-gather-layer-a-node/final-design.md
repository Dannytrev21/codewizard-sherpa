...
- **Internal design:**
  ...
  - **Yarn variant detection (post-precedence) — added per [ADR-0013](ADRs/0013-yarn-variants-as-distinct-package-managers.md):** when the resolved lockfile is `yarn.lock`, run a priority-ordered detection step to emit `yarn-classic` or `yarn-berry` (never the collapsed `yarn`). Priority chain: (1) `package.json#packageManager` matches `^yarn@1\.` → `yarn-classic`; (2) `^yarn@(2|3|4|\d+)\.` → `yarn-berry`; (3) `.yarnrc.yml` exists → `yarn-berry` (Berry-only file extension); (4) `.yarn/` directory exists → `yarn-berry`; (5) `.pnp.cjs` or `.pnp.loader.mjs` exists → `yarn-berry`; (6) default → `yarn-classic` with `node_build_system.yarn_variant_inferred` warning. The distinction is load-bearing: production [ADR-0031](../../production/adrs/0031-plugin-architecture.md) treats `vulnerability-remediation--node--yarn-classic` and `vulnerability-remediation--node--yarn-berry` as distinct plugin scopes because their dependency-resolution models (`node_modules` vs. PnP) are architecturally different. Implemented by story `S2-02a-yarn-variant-detection.md` on top of the shipped S2-02 base.
