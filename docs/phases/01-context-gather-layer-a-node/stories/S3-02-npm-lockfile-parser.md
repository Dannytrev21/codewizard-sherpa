...
**Evidence:** [`_attempts/S3-02.md`](_attempts/S3-02.md) — 15 named unit tests (16 with parametrize) in `tests/unit/probes/_lockfiles/test_npm.py`; full suite 1106 passed / 1 xfail; coverage 94.36%; ruff / ruff-format / mypy --strict / lint-imports clean. One TDD-plan deviation (`os.fstat` monkey-patch → `safe_json.load` monkey-patch with object-identity assertion, mirroring S3-01 Deviation #2) documented in attempt log.
**Effort:** S
...
- [x] **AC-1.** `src/codegenie/probes/_lockfiles/__init__.py` is **not edited** by S3-02. The file content as committed by S3-01 remains byte-for-byte unchanged. Pinned by an architectural test that snapshots the file's content (excluding trailing newline normalization) and compares against the S3-01 baseline (`__all__: list[str] = []` plus the docstring). Extension-by-addition: S3-02 adds **one** new file to `src/`, period.
- [x] **AC-2.** `src/codegenie/probes/_lockfiles/_npm.py` exports exactly `__all__ = ["NpmLock", "parse"]`.
- [x] **AC-3.** `parse(path: Path) -> NpmLock` calls `safe_json.load(path, max_bytes=NPM_LOCKFILE_MAX_BYTES, max_depth=NPM_LOCKFILE_MAX_DEPTH)` with `NPM_LOCKFILE_MAX_BYTES: Final[int] = 50 * 1024 * 1024` and `NPM_LOCKFILE_MAX_DEPTH: Final[int] = 64` declared at module scope (matches arch §"Component design" #9 cap).
- [x] **AC-4.** `parse(path)` **re-raises unchanged** any `SizeCapExceeded`, `DepthCapExceeded`, or `SymlinkRefusedError` raised by `safe_json.load`. No re-wrapping, no swallowing.
- [x] **AC-5.** `parse(path)` translates `MalformedJSONError` raised by `safe_json.load` into `MalformedLockfileError(f"{path}: {type(cause).__name__}: {cause}")` raised `from cause`, so the original `MalformedJSONError` is preserved on `__cause__` (per S1-02 / S3-01 precedent; Rule 12 — fail loud with the chain). The translation covers **all three** `safe_json` paths that raise `MalformedJSONError`: JSON-decode error, empty file, and top-level non-mapping.
- [x] **AC-6.** **Marker contract preserved.** The raised `MalformedLockfileError` carries its message in `args[0]` only; `hasattr(exc, "path")` and `hasattr(exc, "cause")` are both **False**. The Phase 0 invariant `tests/unit/test_errors.py::test_subclasses_are_markers_only` is unaffected by S3-02 code.
- [x] **AC-7.** **`__cause__` chain.** For the malformed-JSON path, `exc.__cause__` is the original `MalformedJSONError` instance (`isinstance(exc.__cause__, MalformedJSONError)` holds; the test pins the class, not the message).
- [x] **AC-8.** **Path observability via message, not attribute.** The raised `MalformedLockfileError`'s `args[0]` contains `str(path)` as a substring (so downstream `WarningId` construction in `NodeManifestProbe` can recover it from the message without instance state).
- [x] **AC-9.** **`NpmLock` `TypedDict` declared `total=False`** with at minimum `name: str`, `version: str`, `lockfileVersion: int`, `requires: bool`, `packages: dict[str, Any]`, `dependencies: dict[str, Any]` — covering lockfileVersion 1/2/3 fields. The `total=False` flag is **load-bearing** — v1 lacks `packages`, v3 lacks `dependencies`; defaulting at the parser layer is forbidden (consumer-side concern per Note #1).
- [x] **AC-10.** **Top-level non-mapping** (JSON list / scalar / `null` document) raises `MalformedLockfileError` via the `MalformedJSONError` translation path (`safe_json._decode` raises `MalformedJSONError("expected JSON object at top level")` for non-dict top-level; `parse()` catches it like any other malformed-JSON and re-raises as `MalformedLockfileError`). Asserts the typed exception, not a `TypeError`.
- [x] **AC-11.** **Empty file** raises `MalformedLockfileError` via the same translation path (`safe_json._decode` raises `MalformedJSONError("empty file")` for zero-byte input; translates to `MalformedLockfileError` per AC-5).
- [x] **AC-12.** **Extension-by-addition.** Adding a future sibling parser (e.g., `_bun.py` for `bun.lockb`) requires **zero edits** to `_npm.py`. Pinned via an architectural test that asserts `_npm.py`'s module text contains no string occurrences of `"_pnpm"`, `"_yarn"`, or `"_bun"` (sibling parsers don't import each other; the kernel they share is `parsers/safe_json.py` + `codegenie.errors`).
- [x] **AC-13.** **Module hygiene.** `ruff format --check`, `ruff check`, `mypy --strict src/codegenie/probes/_lockfiles/_npm.py`, and the unit-test module pass. The cast `cast(NpmLock, raw)` is the **only** runtime-no-op present; no schema validation, no field defaulting, no key reshaping.
- [x] **AC-14.** **TDD discipline.** The red marker commit lands first (`ModuleNotFoundError: codegenie.probes._lockfiles._npm`); each failure-path test asserts the **specific** typed exception class (not just `CodegenieError`); the happy-path test asserts dict-shape, not value-equality of nested structures (that's `NodeManifestProbe`'s concern).
- [x] **AC-15.** **Marker-attribute negative.** A parametrized test asserts that for the `MalformedLockfileError` this module constructs, the caught instance has `args == (some_str,)` and `not hasattr(exc, "path" | "cap" | "detail" | "cause" | "warning_id")` — pins the marker discipline against silent regressions.
