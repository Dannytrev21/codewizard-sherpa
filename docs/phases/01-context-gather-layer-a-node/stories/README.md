...
| S2-02a | [Yarn variant detection — split `yarn` into `yarn-classic` and `yarn-berry` (`S2-02a-yarn-variant-detection`)](S2-02a-yarn-variant-detection.md) | S | S2-02 | Layer the gather-layer fix from [ADR-0013](../ADRs/0013-yarn-variants-as-distinct-package-managers.md) onto the shipped S2-02 base: add `_detect_yarn_variant()` to `node_build_system.py` (priority chain — `package.json#packageManager` field → `.yarnrc.yml` → `.yarn/` dir → `.pnp.cjs` → safe-default-classic-with-warning); update the schema enum from `["bun", "pnpm", "yarn", "npm"]` to `["bun", "pnpm", "yarn-classic", "yarn-berry", "npm"]`; bump `$id` to `v0.2.0`; land two new fixtures (`node_yarn_berry_pnp/`, `node_yarn_berry_nonpnp/`). The distinction is load-bearing for Phase 8's Supervisor (production ADR-0031: `vulnerability-remediation--node--yarn-classic` vs `--yarn-berry` are distinct plugin scopes). **Blocks S3-03** (Berry's lockfile is YAML; Classic's is custom — the parser must branch on the now-distinguished variant). |
