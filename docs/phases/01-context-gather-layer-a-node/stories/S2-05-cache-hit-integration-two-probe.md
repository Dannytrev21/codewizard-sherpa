...
**Status:** Done — 2026-05-14 (phase-story-executor attempt 1, GREEN). All ACs verified with runtime evidence; 2 new integration tests pass (`test_two_probes_cache_hit_on_second_run` + `test_two_probes_cache_miss_on_tracked_input_edit`); full suite **1015 passed** + 1 pre-existing xfail; `ruff check` + `ruff format --check` + `mypy --strict src/codegenie` clean. Attempt log: [`_attempts/S2-05.md`](_attempts/S2-05.md). Implementation: [`tests/integration/probes/test_cache_hit_on_real_repo.py`](../../../../tests/integration/probes/test_cache_hit_on_real_repo.py), conftest extended in [`tests/integration/probes/conftest.py`](../../../../tests/integration/probes/conftest.py) with `_install_scandir_counter` re-export, `_stat_snapshot`, and `WARM_PATH_CACHE_HIT_PROBES` frozenset. Deviations: (1) AC-6 `_stat_snapshot` excludes `<root>/.codegenie/` since the gather legitimately writes there on every run — the fixture-immutability invariant tracks *input* drift, not output (L-28); (2) AC-14/15 slice-content assertions adapted to match the actual envelope shape (only `schema_slice` keys are shallow-merged into `envelope.probes.<name>` — same deviation S2-04 hit at L-25; top-level `errors`/`warnings`/`confidence` are not on the envelope, so fail-loud signals are split across in-slice fields + captured structlog events). Cross-story lessons: L-28.
**Effort:** M
