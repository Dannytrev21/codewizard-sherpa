...
**Status:** Done — 2026-05-14 (phase-story-executor attempt 1, GREEN). All ACs verified with runtime evidence; 2 new integration tests pass; full suite **1013 passed** + 1 pre-existing xfail; `ruff check` + `ruff format --check` + `mypy --strict src/codegenie` clean; pre-commit on touched files clean. Attempt log: [`_attempts/S2-04.md`](_attempts/S2-04.md). Implementation: [`tests/integration/probes/conftest.py`](../../../../tests/integration/probes/conftest.py), [`tests/integration/probes/test_language_detection_warm_path.py`](../../../../tests/integration/probes/test_language_detection_warm_path.py), [`tests/integration/__init__.py`](../../../../tests/integration/__init__.py), [`tests/integration/probes/__init__.py`](../../../../tests/integration/probes/__init__.py); CLI seam wired in [`src/codegenie/cli.py`](../../../../src/codegenie/cli.py) (`_seam_registry_for_task` now returns `default_registry.all_probes()` so `NodeBuildSystemProbe` is dispatched alongside `LanguageDetectionProbe`). Deviations: (1) AC-7 path corrected from `probes.node_build_system.package_manager` → `probes.node_build_system.build_system.package_manager` to match the actual S2-02 schema_slice shape (L-21); (2) AC-8/9 split: top-level `probes.<name>.errors`/`warnings`/`confidence` aren't on the envelope (per the LD + NBS schema_slice shapes) — verified instead via captured structlog events (`probe.failure` absent for both probes; each probe's `probe.success` event reports `confidence == "high"`) plus the in-slice `build_system.warnings == []` (L-21); (3) added a module-local `_stub_node_version_check` helper that rebinds `codegenie.probes.node_build_system._exec` to a `SimpleNamespace` shim raising `ToolMissingError` for `node --version` so the environment-dependent `node.version_declared_resolved_disagree` warning never fires (L-22). Cross-story lessons: L-21, L-22.
**Effort:** S
