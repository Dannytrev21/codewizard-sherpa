...
- [ ] **AC-7.** `tests/fixtures/_yarn_corpus/` contains ≥ 4 curated `yarn.lock` files: (a) `single_entry/` — minimal one-entry; (b) `multi_entry_with_deps/` — multi-entry with `dependencies:` sub-blocks; (c) `multi_spec_shared_header/` — comma-joined specifier-range header (`"foo@^1, foo@^2":`); (d) `empty/` — comments-only, zero entries.
