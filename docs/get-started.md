...
??? question "`semgrep` ran but reports `skipped` / `config_absent`"
    `semgrep` defaults to the `p/nodejs` registry pack, which is fetched over the network. When that ruleset genuinely can't be loaded (offline, registry unreachable), semgrep still exits cleanly having run **zero rules** — the probe reports `outcome: skipped (config_absent)` rather than a misleading clean scan. Point it at a local org ruleset by setting `semgrep_config` in `.codegenie/config.yaml`. Note that a repo-local `.semgrep.yml` is **not** auto-detected — analyzed-repo scanner config is intentionally not trusted.

    **If you are on a codegenie build from before 2026-08-14, treat a `config_absent` here with suspicion.** The rules-loaded count is read from semgrep's `time.rules`, which semgrep only populates when `--time` is passed. The probe did not pass it, so `time.rules` was `[]` on *every* scan and **every** semgrep run was misclassified as `skipped / config_absent` — online or offline — with any real findings silently discarded. Fixed by passing `--time`; a scan that produced findings is now never reported as a vacuous skip. On a current build, `config_absent` alongside `rules_run: 0` means the ruleset really did fail to load.
