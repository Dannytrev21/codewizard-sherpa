...
5. **Extension by addition — no *silent* edits.** Adding a new language, task type, or tool must not require *silent, behaviour-changing* edits to existing plugins or stable existing behaviour. Edits the compiler or a fence fully polices — adding a `Literal` member, a struct field, an import line, a schema `$ref` — are the enforcement mechanism, not violations: they are loud, bounded, and reviewable. A protected surface is a *contract* with a snapshot test (the probe-contract pattern), not a frozen file — files and components stay editable, and only narrow, earned contracts are frozen, provisionally. Genuinely cross-cutting horizontal change goes through the sanctioned **migration** path — a loud, reviewed, all-at-once, conformance-gated sweep — never a silent edit. When a genuinely new cross-task capability first appears, a bounded additive core primitive is allowed only under explicit ADR control and then becomes part of the next stable contract surface. The probe contract in [localv2.md §4](../localv2.md) remains fixed. (See [ADR-0039](adrs/0039-extension-by-addition-allows-bounded-core-primitives.md) and [ADR-0043](adrs/0043-extension-by-addition-means-no-silent-edits.md).)
